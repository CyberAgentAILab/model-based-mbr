...
if __name__ == "__main__":
    ...
    for filename in filtered_files:
        ...
        if algorithm in ["None"]:
            ...
            aed_score = compute_score(df, aed_best, trg, compute_evaluate, src=src_input)
            ...
            ln_aed_best = compute_mbr(matrix=matrix, weights=np.exp(logprob / seq_lengths))
            ln_aed_score = compute_score(df, ln_aed_best, trg, compute_evaluate, src=src_input)

            row = [sample_id, ed_score, ed_best, aed_score, aed_best, ln_aed_score, ln_aed_best]
        else:
            assert False
        rows.append(row)
    
    if algorithm == 'None':
        columns = ['sample_id', "ed_score", "ed_best", "aed_score", "aed_best", 
                                         "ln_aed_score", "ln_aed_best"]
        postfix = ""
    else:
        assert False
